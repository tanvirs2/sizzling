...
if (isset($_POST['btnEditProduct'])) {
    ...
    if ($product_title === '') {
        ...
        if ($_FILES['product_image']['name']) { // Check if image file posted or not
            ...
            @unlink($config['IMAGE_UPLOAD_PATH'] . '/menu_image/' . $dataImage["product_image"]);
        ...
        if ($flag === 0) {
            ...
            if ($resultinsertProduct) {
                //Item option edit start
                $company_name = $_POST["company_name"];
                $company_designation = $_POST["company_designation"];
                $tbl_product_option_id = $_POST["tbl_product_option_id"];
                $total_company_item = count($company_name);
//                debug($company_name);
//                debug($company_designation);
//                exit();
                foreach ($company_name AS $key => $value) {
                    ...
                    $sql_company = "UPDATE tbl_product_option SET $insert_company_array WHERE tbl_product_option_id='$tbl_product_option_id[$key]'";
                    $result_company = mysqli_query($con, $sql_company);
                }
                //Item option edit end
                //Item option add start
                $tbl_product_option_name = $_POST["tbl_product_option_name"];
                $tbl_product_price = $_POST["tbl_product_price"];
                echo $total_option_item = count($tbl_product_option_name);

//                debug($_POST["tbl_product_option_name"]);
//                debug($_POST["tbl_product_price"]);
                foreach ($tbl_product_option_name AS $key2 => $value2) {
                    $insert_option_array = '';
                    $insert_option_array .= 'tbl_product_id = "' . $product_id . '"';
                    $insert_option_array .= ',tbl_product_price = "' . $tbl_product_price[$key2] . '"';
                    $insert_option_array .= ',tbl_product_option_name = "' . $tbl_product_option_name[$key2] . '"';
                    $sql_option = "INSERT INTO tbl_product_option SET $insert_option_array";
                    $result_op = mysqli_query($con, $sql_option);
...
$sqlGetProductOption = "SELECT tbl_product_option_id,tbl_product_price,tbl_product_option_name,tbl_product_id FROM tbl_product_option WHERE tbl_product_id= $product_id";
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                    ...
                                                    <div class="col-md-12">
                                                        <?php if (count($arrayProductOption) > 0): ?>
                                                            ...
                                                            <?php foreach ($arrayProductOption AS $productOption): ?>
                                                                <div id="total_div">
                                                                    <input type="hidden" name="tbl_product_option_id[]" value="<?php echo $productOption->tbl_product_option_id; ?>">
                                                                    <div class="col-md-6">
                                                                        <div class="form-group">
                                                                            <label for="company_name_0">Option Title&nbsp;&nbsp;<strong><span style="color:red;">*</span></strong></label>
                                                                            <input type="text" id="company_name_<?php echo $i; ?>" name="company_name[]" value="<?php echo $productOption->tbl_product_option_name; ?>" class="form-control" />
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-md-5">
                                                                        <div class="form-group">
                                                                            <label for="company_designation_0">Price&nbsp;&nbsp;<strong><span style="color:red;">*</span></strong></label>
                                                                            <input type="number" step="0.01" id="company_designation_<?php echo $i; ?>" name="company_designation[]" value="<?php echo $productOption->tbl_product_price ; ?>" class="form-control" />
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                ...
                                                <div id="option_div">
                                                    <!--                                                    <div class="col-md-6">-->
                                                    <!--                                                        <div class="form-group">-->
                                                    <!--                                                            <label for="tbl_product_option_name_0">Option Title&nbsp;&nbsp;<strong><span style="color:red;">*</span></strong></label>-->
                                                    <!--                                                            <input type="text" id="tbl_product_option_name_" name="tbl_product_option_name[]" value="" class="form-control" />-->
                                                    <!--                                                        </div>-->
                                                    <!--                                                    </div>-->
                                                    <!--                                                    <div class="col-md-5">-->
                                                    <!--                                                        <div class="form-group">-->
                                                    <!--                                                            <label for="company_designation_0">Price&nbsp;&nbsp;<strong><span style="color:red;">*</span></strong></label>-->
                                                    <!--                                                            <input type="number" step="0.01" id="tbl_product_price_" name="tbl_product_price[]" value="" class="form-control" />-->
                                                    <!--                                                        </div>-->
                                                    <!--                                                    </div>-->
                                                </div>
                                            </div>
...
<script type="text/javascript">
    ...
    function GenerateCompanyDesignation() {
        var count = $('div.option_div').length;
        console.log(count);
        var fieldHTML = '';
        fieldHTML += '<div class="option_div">';
        fieldHTML += '<div class="col-md-6">';
        fieldHTML += '<div class="form-group">';
        fieldHTML += '<label>Option Title</label>';
        fieldHTML += '<input type="text" id="tbl_product_option_name_' + count + '" name="tbl_product_option_name[]" value="" class="form-control" />';
        fieldHTML += '</div>';
        fieldHTML += '</div>';
        fieldHTML += '<div class="col-md-5">';
        fieldHTML += '<div class="form-group">';
        fieldHTML += '<label>Price</label>';
        fieldHTML += '<input type="number" step="0.01" id="tbl_product_price_' + count + '" name="tbl_product_price[]" value="" class="form-control" />';
        fieldHTML += '</div>';
        fieldHTML += '</div>';
