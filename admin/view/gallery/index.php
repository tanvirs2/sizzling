...
if (isset($_POST['btnAddBanner'])) {
    ...
    if ($banner_status === '0') {
        ...
        if ($flag == 0) {
            ...
            $sql = "INSERT INTO tbl_gallery SET $custom_array";
            $result = mysqli_query($con, $sql);
            if ($result) {
                $success = 'Information saved successfully';
                $link = baseUrl() . "admin/view/gallery/index.php?success=" . base64_encode($success);
                redirect($link);
            } else {
                if (DEBUG) {
...
if (isset($_POST['DeleteBanner'])) {
    ...
    $sqlImage = "SELECT * FROM tbl_gallery WHERE banner_id=$banner_id";
    $resultImage = mysqli_query($con, $sqlImage);
    $dataImage = mysqli_fetch_array($resultImage);
    @unlink($config['IMAGE_UPLOAD_PATH'] . '/gallery_image/' . $dataImage["banner_image"]);
    $sql = "DELETE FROM tbl_gallery WHERE banner_id=$banner_id";
    $result = mysqli_query($con, $sql);
    if ($result) {
...
<head>
    ...
    <title><?php echo $config['ADMIN_SITE_NAME']; ?> | Gallery</title>
                    ...
                    <div class="panel-group">
                        <div class="panel panel-primary" style="border-color: #6BA756;">
                            ...
                            <div class="panel-body">
                                ...
                                <p>N.B.: Use 365*365 dimension for best showcase</p>
                            </div>
                        </div>
                    </div>
                    <div class="panel-group">
                            ...
                            <div class="panel-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped" id="categoryTable">
                                        ...
                                        <?php
                                        $array = array();
                                        $sql = "SELECT * FROM tbl_gallery ORDER BY banner_id DESC";
                                        $result = mysqli_query($con, $sql);
                                        $total = mysqli_num_rows($result);
                                        $sqlList = "SELECT * FROM tbl_gallery ORDER BY banner_id DESC";
                                        ...
                                        <?php while ($obj = mysqli_fetch_object($resultList)): ?>
                                            <tr>
                                                ...
                                                <td style="width: 30%;">
                                                    <img class="img-responsive" src="<?php echo baseUrl('upload/gallery_image/'); ?><?php echo $obj->banner_image; ?>" style="width: 50%;">
                                                </td>
                                                <td style="width: 20%;">
...
<script type="text/javascript">
    $("#galleryActive").addClass("active");
    $("#galleryActive").parent().parent().addClass("treeview active");
    $("#galleryActive").parent().addClass("in");
</script>
